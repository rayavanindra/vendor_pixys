...
.PHONY: pixysrelease
pixysrelease: $(DEFAULT_GOAL) $(INTERNAL_OTA_PACKAGE_TARGET) $(INTERNAL_UPDATE_PACKAGE_TARGET)
	$(hide) ln -f $(INTERNAL_OTA_PACKAGE_TARGET) $(PIXYS_TARGET_PACKAGE)
	$(hide) $(SHA256) $(PIXYS_TARGET_PACKAGE) > $(PIXYS_TARGET_PACKAGE).sha256sum
	$(hide) ./vendor/pixys/tools/generate_json_build_info.sh $(PIXYS_TARGET_PACKAGE)

	$(hide) ln -f $(INTERNAL_UPDATE_PACKAGE_TARGET) $(PIXYS_TARGET_UPDATEPACKAGE)
	$(hide) $(SHA256) $(PIXYS_TARGET_UPDATEPACKAGE) > $(PIXYS_TARGET_UPDATEPACKAGE).sha256sum
	...
	@echo -e ${CL_CYN}"sha256sum      : "${CL_MAG}" $(shell cat $(PIXYS_TARGET_PACKAGE).sha256sum | awk '{print $$1}')"${CL_RST}
	...
	@echo -e ${CL_CYN}"SHA256         : "${CL_MAG}" $(shell cat $(PIXYS_TARGET_UPDATEPACKAGE).sha256sum | awk '{print $$1}')"${CL_RST} 
