...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/pixys/vars/aosp_target_release 2>/dev/null)
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch pixys_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/PixysOS/$PROJECT
